...
import (
	dpfm_api_caller "data-platform-api-invoice-document-cancels-rmq-kube/DPFM_API_Caller"
...
func callProcess(rmq *rabbitmq.RabbitmqClient, caller *dpfm_api_caller.DPFMAPICaller, conf *config.Conf, msg rabbitmq.RabbitmqMessage) (err error) {
	...
	res, errs := caller.AsyncInvoiceDocumentCancels(accepter, &input, &output, l)
	if len(errs) != 0 {
